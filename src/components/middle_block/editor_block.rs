...
pub fn render_editor_block(f: &mut Frame, area: Rect, app: &mut App) {
    let editor_block = Block::default().borders(Borders::NONE); // Borders are handled by middle_block.rs
    ...
    let final_cursor_y = inner_area.y + cursor_visual_y.saturating_sub(app.editor.scroll_offset_visual_y);
...
impl<'a> Widget for EditorWidget<'a> {
    fn render(self, area: Rect, buf: &mut ratatui::buffer::Buffer) {
        ...
        let (sel_start_byte, sel_end_byte) = selection_range.map_or((usize::MAX, usize::MAX), |(sel_start, sel_end)| {
            (sel_start, sel_end)
        });

        // Get syntax highlighter for the current file
        let syntax: &SyntaxReference = self.app // Use self.app to access App fields
        .highlighter
        .syntax_set
        ...
        let theme_bg = self.app.highlighter.get_background_color(); // Unused
        let _theme_fg = self.app.highlighter.get_foreground_color();

        // Fill background
        for y in 0..area.height {
            for x in 0..area.width {
                buf.get_mut(area.x + x, area.y + y).set_bg(theme_bg);
            }
        }
        ...
        'outer: for logical_line_idx in 0..editor.lines.len() {
            ...
            let highlighted_segments_for_logical_line = self.app.highlighter.highlight_line(line_content, syntax);

            let wrapped_segments = editor.get_visual_lines_for_logical_line(logical_line_idx, content_width);
            ...
            for line_segment in wrapped_segments.iter() {
                if visual_line_y >= editor.scroll_offset_visual_y {
                    ...
                    for (syntect_style, text_from_highlighter) in &highlighted_segments_for_logical_line {
                        ...
                        let line_segment_start_char_in_logical = current_char_offset_in_logical_line;
                        let line_segment_end_char_in_logical = current_char_offset_in_logical_line + line_segment.chars().count();

                        let intersection_start = temp_logical_char_offset.max(line_segment_start_char_in_logical);
                        let intersection_end = (temp_logical_char_offset + segment_char_len).min(line_segment_end_char_in_logical);

                        if intersection_start < intersection_end {
                            // There's an overlap, extract the relevant part
                            let relative_start_in_highlighted_segment = intersection_start - temp_logical_char_offset;
                            let relative_end_in_highlighted_segment = intersection_end - temp_logical_char_offset;

                            let part_to_render_chars: String = segment_chars[relative_start_in_highlighted_segment..relative_end_in_highlighted_segment].iter().collect();
                            let part_to_render_visual_width = part_to_render_chars.width() as u16;

                            // Apply horizontal scrolling
                            let scrolled_part_to_render_chars: String = part_to_render_chars.chars()
                                .skip(editor.scroll_offset_x as usize)
                                .collect();
                            let scrolled_part_to_render_visual_width = scrolled_part_to_render_chars.width() as u16;

                            if current_segment_visual_width + scrolled_part_to_render_visual_width > 0 { // Only add if it's visible after scrolling
                                let mut final_style = base_style;

                                // Apply selection highlighting
                                let part_to_render_byte_offset_start = line_content[..intersection_start].len();
                                let part_to_render_byte_offset_end = line_content[..intersection_end].len();
                                ...
                                spans.push(Span::styled(scrolled_part_to_render_chars, final_style));
                                current_segment_visual_width += scrolled_part_to_render_visual_width;
                            }
                        }
